...
"""
TODO:
- Track viewport position with player from beginning rather than calculate it each frame
- Create isometric rendering mode for combat that draws only the current room scaled up to the viewport
"""

def renderTilemap(dungeon, p, sc, viewport_w, viewport_h, tile_size, surface):
    ...
    # Change so that fov is only calculated when player moves, not every frame
    sc.fov(p[0]//tile_size, p[1]//tile_size, 24, dungeon)
    ...
    for row in range(vp_start_row, vp_end_row):
        for col in range(vp_start_col, vp_end_col):
            ...
            if sc.tile_visibility[i]:
                color = tile_types.tile_colors[map[i]]
            else:
                color = tile_types.tile_colors[tile_types.SHADOW]
